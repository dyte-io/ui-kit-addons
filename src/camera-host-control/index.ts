...
export default class CameraHostToggle {
    ...
    cameraPermissionsStore: DyteStore = undefined;
    ...
    private constructor({targetPresets, hostPresets, addActionInParticipantMenu, actionLevel}: CameraHostToggleProps) {
    ...
    static async init(
        { targetPresets, hostPresets, addActionInParticipantMenu = false, meeting, actionLevel = 'PEER' }: CameraHostToggleProps
    ){
    ...
    isParticipantBlocked(participantId: string) {
        ...
        if (this.cameraPermissionsStore.get('overrides')?.[actionLevelId] !== undefined) {
            return !!(this.cameraPermissionsStore.get('overrides')?.[actionLevelId]);
        }
