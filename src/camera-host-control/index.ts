...
export default class CameraHostToggle {
    ...
    static async init(
        { targetPresets, hostPresets, addActionInParticipantMenu = false, meeting }: CameraHostToggleProps
    ){
    ...
    isParticipantBlocked(participantId: string) {
        ...
        if (this.cameraPermissionsStore.get('overrides')?.[participantId] !== undefined) {
            return !!(this.cameraPermissionsStore.get('overrides')?.[participantId]);
        }
